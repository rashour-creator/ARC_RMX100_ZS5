...
module alb_mss_mem_lat
   #(parameter a_w   = 32,    // Address-width: 32b or 40b
    ...
    reg [9:0]          i_bcfg_lat_d;
    reg                i_bbuf_wr;

    wire [9:0]         i_cfg_lat_w;
    wire [9:0]         i_cfg_lat_r;
    ...
    assign buss_wvalid = busp_wvalid;
    assign busp_wready = buss_wready;
...
generate
    if (dl2 < 2) begin : pass_PROC
        // no latency unit, pass all signals
        assign busp_rvalid = buss_rvalid;
        assign buss_rready = busp_rready;
        ...
        assign i_cfg_lat_w = (cfg_lat_w > 10'd1) ? (cfg_lat_w < dep ? cfg_lat_w : 10'd0 | {dl2{1'b1}}) : 10'd1;
        assign i_cfg_lat_r = (cfg_lat_r > 10'd1) ? (cfg_lat_r < dep ? cfg_lat_r : 10'd0 | {dl2{1'b1}}) : 10'd1;
        ...
        assign busp_rvalid = i_rdel_valid_r[i_rrptr_r];
        assign buss_rready = (i_rcnt_r < i_rcfg_lat_r);
        always @*
          begin : rd_PROC
              // default outputs
              ...
              i_rbuf_wr      = 1'b0;
              // write to fifo
              if (buss_rvalid && buss_rready) begin
        ...
        always @(posedge clk or
          ...
          begin : rdst_PROC
              if (!rst_b) begin
                 ...
                 i_rcfg_lat_r   <= 10'd1; // Set latency to 1 (minimum) after reset
                 i_bdel_valid_r <= {dep{1'b0}};
                 i_bcnt_r       <= {dl2{1'b0}};
                 i_brptr_r      <= {dl2{1'b0}};
                 i_bwptr_r      <= {{dl2-1{1'b0}},1'b1}; // set to latency(=1)
                 i_bcfg_lat_r   <= 10'd1; // Set latency to 1 (minimum) after reset
              end
              else if (clk_en) begin
