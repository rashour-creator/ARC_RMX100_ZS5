...
generate //{
  ...
  if(FIFO_DEPTH == 0) begin: fifo_dep_eq_1//{ pass through
   ...
   else begin//}{// The transparent FIFO (0-depth) with the clock ratio supported
     assign i_occp = 1'b0;
     assign o_occp = 1'b0;
...
// leda NTL_RST03 on
    ...
    assign o_data = r_data & {FIFO_WIDTH{o_valid_r}};
    integer j;
    always @*
    begin : rd_port_PROC
      r_data = {FIFO_WIDTH{1'b0}};
      for(j=0; j<FIFO_DEPTH; j=j+1) begin
        r_data = r_data | ({FIFO_WIDTH{rptr_vec_r[j]}} & rf_r[j]);
      end
    end
