...
module mss_bus_switch_ibp_split
  #(
// leda W175 off
...
generate //{
  if(SPLT_NUM == 1) begin:split_num_eq_1_gen// {
    ...
    reg [SPLT_NUM-1:0] rd_chnl_port_id_ptr_1hot;
    reg [SPLT_NUM-1:0] wrsp_chnl_port_id_ptr_1hot;

    always @ (*) begin : ptr2onehot_PROC
      rd_chnl_port_id_ptr_1hot   = {SPLT_NUM{1'b0}};
      wrsp_chnl_port_id_ptr_1hot = {SPLT_NUM{1'b0}};
      for(j = 0; j < SPLT_NUM; j = j+1) begin//{
        rd_chnl_port_id_ptr_1hot  [j] = (rd_chnl_port_id_ptr   == $unsigned(j));
        wrsp_chnl_port_id_ptr_1hot[j] = (wrsp_chnl_port_id_ptr == $unsigned(j));
      end//}
    end
      ...
      if(ALLOW_DIFF_BRANCH == 1) begin: allow_diff_branch_gen
          ...
          wire to_diff_branch_stall =
             // There is read-outstanding
                 ((~rd_chnl_fifo_empty  ) & (~(rd_chnl_port_id_ptr_1hot == i_ibp_split_indicator)))
             // There is write-outstanding
               | ((~wrsp_chnl_fifo_empty) & (~(wrsp_chnl_port_id_ptr_1hot == i_ibp_split_indicator)));
...
endmodule //mss_bus_switch_ibp_split
