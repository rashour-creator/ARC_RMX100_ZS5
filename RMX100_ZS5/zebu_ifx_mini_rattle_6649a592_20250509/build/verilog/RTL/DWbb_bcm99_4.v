...
//
// Filename    : DWbb_bcm99_4.v
// Revision    : $Id: $
// Author      : Liming SU    03/07/20
// Description : DWbb_bcm99_4.v Verilog module for DWbb
//
// DesignWare IP ID: c46ee712
//
////////////////////////////////////////////////////////////////////////////////
...
// Replace code between "QUADRUPLE FF SYNCHRONIZER BEGIN" and "QUADRUPLE FF SYNCHRONIZER END"
// with one of the following two options of customized register cell(s):
//   Option 1: One instance of a 4-FF cell
//     Macro cell must have an instance name of "sample_meta".
//
//     Example: (TECH_SYNC_4FF is example name of a synchronizer macro cell found in a technology library)
//         TECH_SYNC_4FF sample_meta ( .D(data_s), .CP(clk_d), .RSTN(rst_d_n), .Q(data_d) );
//     
//   Option 2: Four instances of single-FF cells connected serially 
...
//         GTECH_FD2 sample_syncm2 ( .D(n2),     .CP(clk_d), .CD(rst_d_n), .Q(n3)     );
//         GTECH_FD2 sample_syncl  ( .D(n3),     .CP(clk_d), .CD(rst_d_n), .Q(data_d) );
//
//####################### END NOTE ABOUT TECHNOLOGY CELL MAPPING ##########################
// QUADRUPLE FF SYNCHRONIZER BEGIN
  reg sample_meta;
  reg sample_syncm1;
  reg sample_syncm2;
  reg sample_syncl;
...
// spyglass enable_block Reset_sync04
      ...
      sample_syncm2 <= sample_syncm1;
      sample_syncl  <= sample_syncm2;
    end
  end
  assign data_d = sample_syncl;
// QUADRUPLE FF SYNCHRONIZER END
`else
