...
xtor_amba_slave_svs *g_objAmbaSlave;
...
//vector to support out of order transactions from slave
std::vector<ZEBU_IP::XTOR_AMBA_DEFINES_SVS::AXIResp *> rd_resp_vector;
std::vector<ZEBU_IP::XTOR_AMBA_DEFINES_SVS::AXIData *> rd_data_vector;
xtor_amba_slave_svs *saved_xtor;
Board             *board      = NULL;
...
void* zRci_post_board_init(const ZRCI::TbOpts& param) {
      ...
      cerr << "#TB : Register AXI MASTER Transactor..." << param.board << endl;
      xtor_amba_master_svs::Register("xtor_amba_master_axi3_svs");

      cerr << "#TB : Register AXI SLAVE Transactor..." << param.board << endl;
      xtor_amba_slave_svs::Register("xtor_amba_slave_axi3_svs");
     //ZEBU_IP::XTOR_AMBA_SLAVE_SVS::xtor_amba_slave_svs::Register("xtor_amba_slave_svs");
...
cout <<"creating instance for the master here\n";
      objAmba = xtor_amba_master_svs::getInstance("zebu_top.core_chip_dut.icore_sys.izebu_axi_xtor.master_node_i", runtime);

 // Create instance of AMBA slave
cout <<"creating instance for the slave here\n";
      g_objAmbaSlave = xtor_amba_slave_svs::getInstance("zebu_top.core_chip_dut.icore_sys.ialb_mss_dummy_slave.i2_axi_slave.slave_xtor", runtime);
      g_objAmbaSlave->registerReadAddrCB(&cb_raddr);
      g_objAmbaSlave->registerWriteTxnCB(&cb_wrtxn);
     // Set the verbosity of the display messages to be displayed runtime.
     g_objAmbaSlave->setDebugLevel(DEBUG_NONE);
...
void* zRci_cleanup(const ZRCI::TbOpts& param) {
    ...
    // </REMOTE_AXI>
     ...
     if(objAmba != NULL)
     delete objAmba;
    if (g_objAmbaSlave != NULL)
     {
      delete g_objAmbaSlave;
     }
      threading = NULL;
      xsched = NULL;
      XtorSchedParas = NULL;
