...
alb_mss_mem_bypbuf # (
  .BUF_DEPTH(CMD_CHNL_FIFO_DEPTH),
  .BUF_WIDTH((CMD_CHNL_W + RGON_W + USER_W))
) cmd_chnl_fifo(
...
alb_mss_mem_bypbuf # (
  .BUF_DEPTH(WDATA_CHNL_FIFO_DEPTH),
  .BUF_WIDTH(WD_CHNL_W)
) wdata_chnl_fifo(
...
//
alb_mss_mem_bypbuf # (
  .BUF_DEPTH(RDATA_CHNL_FIFO_DEPTH),
  .BUF_WIDTH(RD_CHNL_W)
) rdata_chnl_fifo(
...
alb_mss_mem_bypbuf # (
  .BUF_DEPTH(WRESP_CHNL_FIFO_DEPTH),
  .BUF_WIDTH(WRSP_CHNL_W)
) wresp_chnl_fifo(
...
wire cmd_wait_wd_cnt_inc = i_ibp_cmd_chnl_valid & i_ibp_cmd_chnl_accept & (~i_ibp_cmd_chnl[CMD_CHNL_READ]);
...
wire cmd_wait_wd_cnt_dec = (i_ibp_wd_chnl_valid & i_ibp_wd_chnl_accept & i_ibp_wd_chnl[WD_CHNL_LAST]);
...
// The ibp wr burst counter increased when a IBP write command accepted to the cmd stage
// also note no overflow allowed
wire o_cmd_wait_wd_cnt_inc = o_ibp_cmd_chnl_valid & o_ibp_cmd_chnl_accept & (~o_ibp_cmd_chnl[CMD_CHNL_READ]);
// The ibp wr burst counter decreased when a last beat of IBP write burst accepted to wd stage
// also note no underflow allowed
wire o_cmd_wait_wd_cnt_dec = (o_ibp_wd_chnl_valid & o_ibp_wd_chnl_accept & o_ibp_wd_chnl[WD_CHNL_LAST]);
...
endmodule // alb_mss_mem_ibp_bypbuf
